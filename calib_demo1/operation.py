...
def world_start(request):
	...
	for key in request.GET:
		if request.GET[key] == '':
			message = '请填写完整信息'
			break
	if 'world_name' in request.GET and message == '':
		message = '你选择了'+str(request.GET['world_name'].encode('utf-8')) + ',你的信息录入完成。谢谢配合~'
		test1 = CMD_CONTENT(world_name = request.GET['world_name'])
		test1.save()
